...
// Names/company come from tenant data or Google profiles — escape before they
// land in email HTML so a stray angle bracket can't break the markup.
const esc = (s: string) =>
...
export function buildEstimateEmail(clientName: string, total: number, submissionId: string, tenant: TenantConfig, tradeLabel: string): string {
  return `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
      <h2 style="color: ${tenant.primary_color};">${esc(tenant.company_name)}</h2>
      <h3>${esc(tradeLabel)} Estimate</h3>
      ...
      <p style="color: #374151;">Hi ${esc(clientName)},</p>
      <p style="color: #374151;">Please find your estimate attached. This is based on the scope discussed during our walkthrough.</p>
      ...
      <p style="font-size: 12px; color: #9ca3af;">
        Reference: ${submissionId.slice(0, 8).toUpperCase()} | ${esc(tenant.company_name)}${tenant.contact_phone ? ` | ${esc(tenant.contact_phone)}` : ''}
      </p>
    </div>
  `;
}
...
export function buildMagicLinkEmail(firstName: string, loginUrl: string, brandName?: string, brandColor?: string): string {
  const name = brandName || 'GuildQuote';
  const color = brandColor || '#2563eb';
  return `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
      <h2 style="color: ${color};">${esc(name)}</h2>
      ...
      <hr style="border: none; border-top: 1px solid #e5e7eb; margin: 20px 0;" />
      <p style="font-size: 12px; color: #9ca3af;">${esc(name)}</p>
    </div>
  `;
}
...
export function buildWelcomeEmail(companyName: string, loginUrl: string): string {
  return `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
      <h2 style="color: #2563eb;">Welcome to GuildQuote!</h2>
      <p>Hi there,</p>
      <p>Your account for <strong>${esc(companyName)}</strong> has been created. Here's what to do next:</p>
      ...
      <p style="font-size: 12px; color: #9ca3af;">GuildQuote | Estimate generation for contractors</p>
    </div>
  `;
}
