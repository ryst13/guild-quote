...
export const users = sqliteTable('users', {
  ...
  phone: text('phone'),
  role: text('role', { enum: ['contractor_admin', 'contractor_staff', 'homeowner'] }).notNull().default('homeowner'),
  created_at: text('created_at').notNull().$defaultFn(() => new Date().toISOString()),
});
...
export const submissions = sqliteTable('submissions', {
  ...
  rooms_json: text('rooms_json').notNull(),
  quote_json: text('quote_json'),
  sales_price: real('sales_price'),
  stage_key: text('stage_key').notNull().default('intake'),
  estimator_notes: text('estimator_notes'),
  estimator_approved: integer('estimator_approved', { mode: 'boolean' }).default(false),
  client_accepted: integer('client_accepted', { mode: 'boolean' }).default(false),
