<script lang="ts">
  ...
  let { onSubmit, demo = false, colorSamplesAmount = 98.95, transportationAmount = 50 , recentClients = [] }: { onSubmit: (data: InteriorScopeData) => void; demo?: boolean; colorSamplesAmount?: number; transportationAmount?: number; recentClients?: { name: string; email: string; phone: string; address: string }[] } = $props();
